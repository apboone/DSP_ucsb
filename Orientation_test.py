...
def getLineInfo(line): #returns list of all info
    ...
    Orientation = float(split_other[2])
    info = [Time_Elapsed, X_Coord, Y_Coord, Orientation]
    print (info)
    return info
...
def getOrientation(line):
    ...
    Orientation_type = 0
    if Orientation >= 135.0001 and Orientation <= 224.9999:
    	Orientation_type = 3
    if Orientation >= 315.0001 and Orientation <= 44.9999:
    	Orientation_type = 1
    if Orientation  >= 225.0001 and Orientation <= 314.9999:
    	Orientation_type = 4
    if Orientation >= 45.0001 and Orientation <= 134.9999:
    	Orientation_type = 2
...
if (Strategy_Predictor == True):
    ...
    wb = xlsxwriter.Workbook("/Users/alexanderboone/Desktop/test/Steps_Counter_OutPuts/Orientation_Test.xlsx")
                ...
                for current_line in infile:
                    if (numOfLines <= 7): #this depends on which headers the file has...
                        ...
                        if "b" in current_line:
                            ...
                            line2 = current_line.split(",")
                            #print(line2)


                            curr_Orient = getOrientation(line2[2])
                            
                            box = [curr_X, curr_Y, curr_Orient]
                            print(box)
                            #print(steps)
